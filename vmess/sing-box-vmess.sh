...
install_singbox(){
    ...
    sed -i "s/公钥/$public_key/g" /root/sing-box/client-sockshttp.json
    ...
    if [[ -n $(service sing-box status 2>/dev/null | grep "inactive") ]]; then
        ...
        yellow "clash.meta 配置 /n {name: singbox_reality, type: vmess, server: $v4, port: $custom_port, uuid: $ps, alterId: 0, cipher: none, network: tcp, udp: true, client-fingerprint: chrome, servername: $tlsdomain, reality-opts: {public-key: $public_key}}"
    fi
}
...
menu(){
    clear
    echo "#####################################################################################"
    echo -e "#              ${RED} Sing-box+vmess+reality  一键管理脚本${PLAIN}            #"
    echo "#####################################################################################"
    echo ""
